...
import { 
  Building, CheckCircle2, AlertTriangle, Key, Trash2, 
  RefreshCw, CheckCircle, Clock, Link2, ExternalLink
} from "lucide-react";
import { connectWindsor, disconnectWindsor, triggerManualSync } from "../../server/actions/windsor";
...
export function IntegrationsView({ connection }: IntegrationsViewProps) {
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      const res = await connectWindsor(apiKey);
      if (res.error) {
        setError(res.error);
      } else {
        setApiKey("");
  ...
  const handleDisconnect = async () => {
    if (!confirm("Are you sure you want to disconnect Windsor.ai? All synced campaign metric history will remain but no new syncs will trigger.")) return;
    setLoading(true);
    
    try {
      await disconnectWindsor();
      router.refresh();
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        alert(`Sync finished successfully! Synced ${res.rowsSynced} records.`);
        router.refresh();
      }
    } catch (err: any) {
  ...
  const connectorsList = [
    { name: "Google Ads", desc: "Search & Display performance metrics", active: !!connection },
    { name: "Meta Ads", desc: "Facebook & Instagram campaign performance", active: !!connection },
    { name: "LinkedIn Ads", desc: "B2B audience targeting & conversion tracking", active: !!connection },
    { name: "Microsoft Ads", desc: "Bing search Network & display metrics", active: !!connection },
  ];

  return (
    <div className="space-y-8 max-w-4xl">
      {/* Active Connection state panel */}
      {connection ? (
        <div className="glass-card rounded-[24px] border border-white/5 bg-white/[0.01] p-6 space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400 font-light mt-0.5">
                  Synced status: <span className="font-medium text-emerald-400 uppercase">{connection.syncStatus}</span>
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 border-t border-white/[0.03] pt-6">
            <div>
              <span className="block text-[10px] text-neutral-500 uppercase font-light">Connected On</span>
              <span className="text-xs text-neutral-300 mt-1 block font-mono">{formatDateTime(connection.createdAt)}</span>
            </div>
            <div>
              <span className="block text-[10px] text-neutral-500 uppercase font-light">Last Synchronization</span>
              <span className="text-xs text-neutral-300 mt-1 block font-mono">{formatDateTime(connection.lastSyncAt)}</span>
            </div>
            <div>
              <span className="block text-[10px] text-neutral-500 uppercase font-light">Background Frequency</span>
              <span className="text-xs text-neutral-300 mt-1 block font-medium text-purple-400">Every 15 Minutes</span>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="glass-card rounded-[24px] border border-white/5 bg-white/[0.01] p-8 space-y-6">
          <div className="max-w-md">
            ...
            <p className="text-xs text-neutral-400 mt-1.5 font-light leading-relaxed">
              PerformanceOS AI requires an active Windsor.ai API key to query and aggregate multi-channel marketing campaigns. Enter your credential below to activate the dash.
            </p>
          </div>
          ...
          <form onSubmit={handleConnect} className="flex flex-col sm:flex-row gap-3 max-w-lg">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Enter Windsor.ai api_key"
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 rounded-[12px] bg-white text-black font-medium hover:bg-neutral-200 active:scale-95 transition text-xs cursor-pointer flex-shrink-0"
            >
      ...
      {/* Available Channels */}
      <div>
        <h3 className="font-display font-medium text-sm text-white mb-4">Supported Connectors</h3>
            ...
            <div key={c.name} className="glass-card rounded-[20px] p-5 border border-white/5 bg-white/[0.01] flex items-start gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {c.active && (
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" title="Connected & Active" />
                  )}
                </div>
      ...
      {/* Connection Logs */}
      {connection && connection.syncLogs && (
        <div>
          <h3 className="font-display font-medium text-sm text-white mb-4">Sync History Logs</h3>
                  ...
                  {connection.syncLogs.map((log) => (
                    <tr key={log.id} className="text-neutral-300">
                      ...
                      <td className="py-3.5 px-5">
                        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-medium tracking-wide ${
                          log.status === "SUCCESS"
                      ...
                      <td className="py-3.5 px-5 text-neutral-500 font-light truncate max-w-[200px]" title={log.errorMessage || "Completed successfully"}>
