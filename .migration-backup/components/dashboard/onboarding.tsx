...
import { Key, Sparkles, RefreshCw, AlertTriangle, ShieldCheck, ArrowRight, CheckCircle2 } from "lucide-react";
import { connectWindsor, triggerManualSync } from "../../server/actions/windsor";
...
export function OnboardingPanel() {
  ...
  const handleSync = async () => {
    setLoading(true);
    setError(null);
    setSyncLogs((prev) => [...prev, "Contacting Windsor.ai...", "Fetching ad campaign records..."]);

    try {
      const res = await triggerManualSync();
      if (res.error) {
        setError(res.error);
        setSyncLogs((prev) => [...prev, `Sync failed: ${res.error}`]);
      } else {
        setSyncLogs((prev) => [
          ...prev,
          `Successfully fetched campaign metrics.`,
          `Synchronized ${res.rowsSynced} daily metric records.`,
      ...
      setSyncLogs((prev) => [...prev, "Fatal error occurred during synchronization."]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center items-center py-10 px-4">
      ...
      <motion.div
        ...
        className="glass-card w-full max-w-[620px] rounded-[24px] border border-white/5 bg-white/[0.01] backdrop-blur-[24px] p-8 md:p-10 shadow-2xl relative z-10"
      >
        {/* Step Indicator */}
        ...
        {step === 1 ? (
          <div>
            ...
            <div className="mb-8">
              ...
              <h2 className="text-xl md:text-2xl font-display font-medium text-white tracking-tight">
                Connect your Windsor.ai Data Layer
              </h2>
            ...
            <form onSubmit={handleConnect} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Enter api_key (e.g. 5d5a7b8c...)"
                    className="w-full pl-10 pr-4 py-3 rounded-[12px] bg-white/[0.02] border border-white/5 focus:border-blue-500/40 focus:bg-white/[0.04] focus:ring-1 focus:ring-blue-500/40 transition duration-200 text-sm placeholder:text-neutral-600 outline-none"
                  />
                </div>
                <span className="block text-[11px] text-neutral-500 mt-2 font-light leading-normal">
                  You can find your API key inside the Windsor.ai Dashboard under the "API" tab.
                </span>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-[12px] bg-white text-black font-medium hover:bg-neutral-200 active:scale-[0.98] transition duration-200 text-sm disabled:opacity-50 cursor-pointer"
              >
          ...
          <div>
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-neutral-400 mt-2 font-light leading-relaxed">
                Your Windsor.ai connection is verified. We will now fetch the last 30 days of campaign performance data across all connected marketing channels. No mock or demo data will be used.
              </p>
            </div>
            ...
            <button
              onClick={handleSync}
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-[12px] bg-purple-600 hover:bg-purple-500 text-white font-medium active:scale-[0.98] transition duration-200 text-sm disabled:opacity-50 cursor-pointer shadow-lg shadow-purple-900/10"
            >
              {loading ? (
                <>
