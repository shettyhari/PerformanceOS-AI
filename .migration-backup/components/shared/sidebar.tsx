...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-white/[0.04]">
        <Link href="/dashboard" className="flex items-center gap-3 outline-none">
          <div className="h-9 w-9 flex-shrink-0 rounded-[10px] bg-gradient-to-tr from-blue-600 via-purple-600 to-red-500 flex items-center justify-center shadow-lg shadow-purple-900/10">
            <span className="font-display font-bold text-sm tracking-tight text-white">P</span>
          </div>
          {!isCollapsed && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              <span className="font-display font-medium text-sm tracking-tight text-white leading-tight">
                PerformanceOS
              </span>
              <span className="text-[10px] text-neutral-500 font-light leading-tight">
                AI Marketing Suite
              </span>
            </motion.div>
          )}
        ...
        {!isCollapsed && (
          <button 
            ...
            className="p-1.5 rounded-[8px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.06] hover:text-white transition duration-200 cursor-pointer"
          >
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href} className="outline-none block">
              <span
                className={`flex items-center gap-3 px-3 py-2.5 rounded-[12px] transition duration-200 relative group cursor-pointer ${
                  isActive 
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute left-0 w-1 h-5 bg-gradient-to-b from-blue-500 to-purple-600 rounded-r-full"
                ...
                <Icon className={`w-5 h-5 flex-shrink-0 transition-colors ${
                  isActive 
                    ? item.isAi ? "text-purple-400" : "text-white"
                    : item.isAi ? "text-purple-500 group-hover:text-purple-400" : "text-neutral-500 group-hover:text-neutral-300"
                }`} />
                ...
                {!isCollapsed && item.isAi && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded-[6px] bg-purple-950/40 border border-purple-500/20 text-purple-300 font-medium">
                    AI
                  </span>
                )}
              </span>
      ...
      {isCollapsed && (
        <div className="py-2 flex justify-center border-t border-white/[0.04]">
          <button 
            onClick={toggleSidebar} 
            className="p-1.5 rounded-[8px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.06] hover:text-white transition duration-200 cursor-pointer"
          >
      ...
      <div className="p-3 border-t border-white/[0.04] bg-[#020203]/20">
        <div className="flex items-center gap-3 justify-between">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-blue-900/60 to-purple-900/60 border border-white/5 flex items-center justify-center flex-shrink-0 text-white font-medium text-xs">
          ...
          {!isCollapsed && (
            <button
              ...
              className="p-1.5 rounded-[8px] text-neutral-500 hover:text-red-400 hover:bg-red-950/20 transition duration-200 cursor-pointer"
              title="Sign Out"
            >
