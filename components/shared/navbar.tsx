...
import { RefreshCw, CheckCircle2, Shield, LayoutGrid, ChevronDown, Building, RefreshSlide } from "lucide-react";
...
export function Navbar({ orgName, role }: NavbarProps) {
  ...
  return (
    <header className="h-16 flex items-center justify-between px-6 border-b border-white/[0.04] bg-[#050508]/40 backdrop-blur-[12px] z-10">
      {/* Left side: Org Switcher */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-[12px] bg-white/[0.02] border border-white/5 hover:border-white/10 active:scale-98 transition duration-200 cursor-pointer">
      ...
      {/* Right side: Sync State, Theme Toggle, Profile */}
      <div className="flex items-center gap-3">
        {/* Sync Status Button */}
        <div className="flex items-center gap-2 mr-2">
          <span className="text-[11px] text-neutral-500 hidden md:inline font-light">
            {lastSynced}
          </span>
          <button
        ...
        <div 
          className="h-9 w-9 rounded-[12px] bg-white/[0.02] border border-white/5 flex items-center justify-center text-neutral-300 hover:text-white cursor-pointer"
          title="Windsor.ai Data Layer: Healthy"
        >
