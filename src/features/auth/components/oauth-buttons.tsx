...
export type OAuthProvider = 'google' | 'facebook' | 'linkedin' | 'azure-ad';
  ...
  if (providers.length === 0) {
    return (
      <div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">
        No OAuth providers configured. Set provider credentials in your environment.
      </div>
    );
  }
      ...
      {providers.map((provider) => {
        ...
        return (
          <Button
            ...
            type="button"
            variant="outline"
            className={cn('h-11 w-full justify-start gap-3', config.className)}
